...
# Load an image file (whose name is provided as the first command-line 
# argument) into a numpy array using the scipy.misc.imread function.
...
# Keep only the top-k entries in the SVD decomposed matrices and multiply them
# to obtain the best rank-k approximation of the original array (the k value 
# should come from the second command-line argument of task1.py)
S = numpy.diag(s[:rank])
kApprox = numpy.dot(U[:, :rank], numpy.dot(S, V[:rank, :]))

# Save the approximated array as (1) a binary array file (with the name from 
# the getOutputNpyName function call) using numpy.save function and (2) a PNG 
# file (with the name from the getOutputPngName function call) using the 
# scipy.misc.imsave function.
