...
public class SpellChecker2 {
    ...
    public double calculateEM(String word, String correction, double editDistance) {
        ...
        // More weight given if the smaller the edit distance
        ...
        // Since vowels are the most likely to be misused, more weight given if
        // the strings are the same minus the vowels
        ...
        for(int j = 0; j < correction.length(); ++j) {
            if(correction.charAt(j) != 'a' && correction.charAt(j) != 'e' && 
               correction.charAt(j) != 'i' && correction.charAt(j) != 'o' &&
               correction.charAt(j) != 'u' && correction.charAt(j) != 'y') {
                correctionSB.append(correction.charAt(j));
            }
        }

        // More weight equal    
        if(wordSB.equals(correctionSB)) {
            em *= 100;
        }

        // Sound-alike letters [c, s] and [j, g] can also contribute to errors
        // If we check for these swapped letters we can catch errors
        for(int k = 0; k < word.length() && k < correction.length(); ++k) {
            if(word.charAt(k) == 's' && correction.charAt(k) == 'c' ||
               word.charAt(k) == 'c' && correction.charAt(k) == 's' ||
               word.charAt(k) == 'j' && correction.charAt(k) == 'g' ||
               word.charAt(k) == 'g' && correction.charAt(k) == 'j') {
                em *= 100;
            }
        }

        // Silent letters: more likely to spell wrong if using silent letters 
        ...
        // Double consonants edits should be fairly common, and we should give 
        // weight to those as well
        if(correction.contains("cc")) {
            em *= 100;
        }
        if(correction.contains("dd")) {
            em *= 100;
        }
    ...
    */
    private Map < String, Double > EditDist1(String word) {
        ...
        for (int i = 0; i <= word.length(); ++i) {
            ...
            // "transposition at [i]" swap current position i with position i - 1
            if (R.length() > 0 && L.length() > 0) {
                temp = R.substring(0, 1) + L.substring(L.length() - 1);
                if(L.length() > 1) {
                    temp = L.substring(0, L.length() - 1) + temp;
                }
                if (R.length() > 1) {
                    temp = temp + R.substring(1);
                }
                cands.put(temp, 1.0); // 1.0 indicates the edit distance 1
            }
            // TODO: implement substitution operator here
            // "substitution at [i]" substitute an alphabet (a-z) at the current position i
            for(int k = 0; k < alphabet.length(); ++k) {
                if(R.length() > 0) {
                    temp = L + alphabet.charAt(k) + R.substring(1);
                    cands.put(temp, 1.0); // 1.0 indicates the edit distance 1
                }
            }
        }
